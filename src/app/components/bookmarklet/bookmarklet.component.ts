...
export class BookmarkletComponent {
  ...
  public readonly mode = input<'github' | 'local'>('github');

  protected readonly modified: Record<'github' | 'local', string> = {
    github: '2026-07-28',
    local: '2026-07-29',
  };
  ...
  protected readonly projectBookmarklet = computed(() => {
    return this.mode() === 'github' ? this.ghBookmarklet() : this.utBookmarklet();
  });

  private readonly ghBookmarklet = computed(() => {
  ...
  private readonly utBookmarklet = computed(() => {
    const repo = this.projectData().github.repo;
