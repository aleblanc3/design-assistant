...
export class ExportComponent implements OnInit {
  ...
  async compareFiles() {
    ...
    const githubPages: Map<string, string> = owner && this.gitHubData().repo ? await this.exportGitHubService.getRepoTree(owner, repo, branch, token) : new Map();
  ...
  async exportToFile() {
    ...
    const projectPaths = this.projectTable()
      .filter((item) => item.status === ExportStatus.ExportNew || item.status === ExportStatus.ExportOverwrite)
      .map((item) => item.path);
    const templatePaths = this.templateTable()
      .filter((item) => item.status === ExportStatus.ExportNew || item.status === ExportStatus.ExportOverwrite)
      .map((item) => item.path);
    ...
    for (const path of projectPaths) {
      ...
      const breadcrumbs = this.projectState
        .getBreadcrumbChain(path, lang)
        .map((b) => `{ title: "${b.title}", href: "${b.link}" }`)
        .join(', ');

      const header = doubleH1
        ? `<p class="lead mrgn-tp-md mrgn-bttm-0 text-muted">${doubleH1}</p>
<h1 property="name" id="wb-cont" dir="ltr" class="mrgn-tp-0">${h1}</h1>`
        : `<h1 property="name" id="wb-cont" dir="ltr">${h1}</h1>`;
      ...
      try {
        ...
        const doc = !source.endsWith('UT') ? await this.fetchService.fetchContent(url, 'both', retries) : this.fetchService.stringToDoc(await this.fetchService.fetchViaProxy(url));
        if (!doc) {
    ...
    this.usageService.trackExport(
      this.projectData().id,
      this.projectData().org ?? 'DEFAULT',
      this.projectData().storageType,
      this.projectData().repoType,
      `${repo}`,
      this.projectCache.selectedVersion(),
      pageCountEN,
      pageCountFR,
    );
    //Download zip file
  ...
  buildCdtsIndex(paths: Set<string>) {
    ...
    const collaboratorHtml = collaboratorNames
      ? `<section class="gc-contributors">
         ...
         <ul>${this.projectData()
           .collaborators.map((collab) => ` <li> ${collab.login}</li>`)
           .join('')}</ul>
         </section>\n`
      : '';
    ...
    //Table headers
    const headers =
      this.projectCache.selectedLang() === 'en'
    ...
    const buildRows = (pairsList: typeof exportedPairs) =>
      pairsList
        .map((pair) => {
      ...
      try {
        ...
        if (!isSkipped) {
          if (isNew) {
            ...
            const doc = !source.endsWith('UT') ? await this.fetchService.fetchContent(url, 'both') : this.fetchService.stringToDoc(await this.fetchService.fetchViaProxy(url));
            const breadcrumbs = this.projectCache.selectedVersion() === 'prototype' ? this.projectState.getBreadcrumbChain(node.data.path[lang], lang).slice(1) : undefined; //baseline uses live breadcrumb
  ...
  async exportProjectToGitHub() {
    ...
    const allPagePaths = new Set([...enPages.map((page) => page.path), ...frPages.map((page) => page.path), ...githubContentPages]);
    // Format redirect
    const redirects = [...allPagePaths].map((path) => ({
      origin: `https://www.canada.ca/${path}`,
